# Community Research Audit — The Future of Human Perception
...
| `polygraphene/ALVR` | — | C++ | VR streaming — "missing latency" finding |
...
| 4 | Missing docs on HoloLens 2 mixed reality capture | `MixedRealityToolkit/MixedRealityToolkit-Unity #987` | 4 | Even Microsoft's own MRTK can't document how to turn on MRC for HoloLens 2. |
...
| **fieldsJacksonG** | Microsoft MRC | Hologram registration & calibration (#221, #228) |
...
### 3. Spatial Audio Is Architecturally Absent from WebXR
...
1. **Repo Discovery**: Searched GitHub with `topic:augmented-reality`, `topic:mixed-reality`, `topic:spatial-computing`, `topic:webxr`, `topic:spatial-audio` (stars-filtered)
2. **Issue Mining**: Searched issues across all surveyed repos for keywords: latency, perceptual, spatial audio, HRTF, mixed reality, hologram, drift, registration, foveation, wayfinding
3. **Contributor Extraction**: Identified repo owners, assignees, and active commenters on the hottest issues
4. **Debate Synthesis**: Analyzed issue comment threads to extract the core perceptual science questions being debated
5. **Cross-Referencing**: Matched interview candidates to specific GitHub activity (not just follower counts)

---

*Last updated — September 2026. This is a living document — submit PRs to update as new issues emerge and new contributors join the space.*
